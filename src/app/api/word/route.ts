...
import { WordCreateViewModel, WordUpdateViewModel } from "~/DAL/viewModels/WordViewModel";
...
export async function PUT(request: NextRequest) {
    try {
        ...
        const viewModel = await request.json() as WordUpdateViewModel;

        const data = {
            id: viewModel.id ?? Number(paramId),
            word: viewModel.word,
            type: viewModel.type,
            wordFamily: viewModel.wordFamily,
        } as Word;
        ...
        const model = await service.Update(data);
        return NextResponse.json({ success: true, data: model }, { status: 200 });
    } catch (e) {
        console.error(e);
        return NextResponse.json({ success: false }, { status: 500 });
    }
